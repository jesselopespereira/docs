...
<ResponseField name="url" type="string">
	...
	``` http://localhost:3000/public/a1b2c3d4e5f6.png ```
</ResponseField>

<ResponseField name="bbox" type="object">
	Bounding box of the generated image.
	<Expandable title="BBox structure">
		```json
		{
			"minLat": -23.56,
			"maxLat": -23.54,
...
<ResponseField name="xlim" type="number[]">
	Longitude limits [min, max].
</ResponseField>

<ResponseField name="ylim" type="number[]">
...
<ResponseField name="200" type="Success">
	Map successfully generated. Returns object with `url`, `bbox`, `xlim` and
	`ylim`.
</ResponseField>
...
The API uses the following default parameters for the Kriging algorithm if not provided in the request:

| Parameter       | Default Value | Description                                              |
| --------------- | ------------- | -------------------------------------------------------- |
| Variogram Model | `exponential` | Mathematical model used to calculate spatial correlation |
| Sigma²          | `0`           | Variance of measurement error                            |
...
- Images are saved in `/public` with the name `{hash}.png`
